...
public class XXHashPartitioner implements IPartitioner {
    public static final XXHashPartitioner.LongToken MINIMUM = new XXHashPartitioner.LongToken(Long.MIN_VALUE);
    ...
    private final Splitter splitter = new Splitter(this)
    {
        ...
        {
            return new XXHashPartitioner.LongToken(value.longValue());
        }

        public BigInteger valueForToken(Token token)
        {
            return BigInteger.valueOf(((XXHashPartitioner.LongToken) token).token);
        }
    };

    public DecoratedKey decorateKey(ByteBuffer key)
    {
        return new BufferDecoratedKey(getToken(key, getHash(key)), key);
    }

    public Token midpoint(Token lToken, Token rToken)
    {
        // using BigInteger to avoid long overflow in intermediate operations
        BigInteger l = BigInteger.valueOf(((XXHashPartitioner.LongToken) lToken).token),
                r = BigInteger.valueOf(((XXHashPartitioner.LongToken) rToken).token),
                midpoint;
        ...
        return new XXHashPartitioner.LongToken(midpoint.longValue());
    }

    public Token split(Token lToken, Token rToken, double ratioToLeft)
    {
        BigDecimal l = BigDecimal.valueOf(((XXHashPartitioner.LongToken) lToken).token),
                r = BigDecimal.valueOf(((XXHashPartitioner.LongToken) rToken).token),
        ...
        return new XXHashPartitioner.LongToken(newToken);
    }

    public XXHashPartitioner.LongToken getMinimumToken()
    {
        return MINIMUM;
    }
    ...
    public static class LongToken extends Token
    {
        ...
        public boolean equals(Object obj)
        {
            ...
            return token == (((XXHashPartitioner.LongToken)obj).token);
        }
        ...
        {
            return Long.compare(token, ((XXHashPartitioner.LongToken) o).token);
        }
        ...
        {
            XXHashPartitioner.LongToken n = (XXHashPartitioner.LongToken) next;
        ...
        {
            return new XXHashPartitioner.LongToken(token + 1);
        }
    }
     ...
     */
    public XXHashPartitioner.LongToken getToken(ByteBuffer key)
    {
        return getToken(key, getHash(key));
    }

    private XXHashPartitioner.LongToken getToken(ByteBuffer key, long hash)
    {
        if (key.remaining() == 0)
            return MINIMUM;

        return new XXHashPartitioner.LongToken(normalize(hash));
    }
    ...
    public XXHashPartitioner.LongToken getRandomToken()
    {
        return getRandomToken(ThreadLocalRandom.current());
    }

    public XXHashPartitioner.LongToken getRandomToken(Random r)
    {
        return new XXHashPartitioner.LongToken(normalize(r.nextLong()));
    }
    ...
    public Map<Token, Float> describeOwnership(List<Token> sortedTokens)
    {
        ...
        else
        {
            ...
            Token start = i.next();BigInteger ti = BigInteger.valueOf(((XXHashPartitioner.LongToken)start).token);  // The first token and its value
            Token t; BigInteger tim1 = ti;                                                                // The last token and its value (after loop)

            while (i.hasNext())
            {
                t = i.next(); ti = BigInteger.valueOf(((XXHashPartitioner.LongToken) t).token); // The next token and its value
            ...
            float x = new BigDecimal(BigInteger.valueOf(((XXHashPartitioner.LongToken)start).token).subtract(ti).add(ri).mod(ri)).divide(r, 6, BigDecimal.ROUND_HALF_EVEN).floatValue();
    ...
    private final Token.TokenFactory tokenFactory = new Token.TokenFactory()
    {
        ...
        {
            XXHashPartitioner.LongToken longToken = (XXHashPartitioner.LongToken) token;
        ...
        {
            out.writeLong(((XXHashPartitioner.LongToken) token).token);
        }

        @Override
        public void serialize(Token token, ByteBuffer out)
        {
            out.putLong(((XXHashPartitioner.LongToken) token).token);
        }
        ...
        {
            return new XXHashPartitioner.LongToken(ByteBufferUtil.toLong(bytes));
        }

        @Override
        public Token fromByteBuffer(ByteBuffer bytes, int position, int length)
        {
            return new XXHashPartitioner.LongToken(bytes.getLong(position));
        }
        ...
        public Token fromString(String string)
        {
            try
            {
                return new XXHashPartitioner.LongToken(Long.parseLong(string));
            }
            catch (NumberFormatException e)
            {
                throw new IllegalArgumentException(String.format("Invalid token for XXHashPartitioner. Got %s but expected a long value (unsigned 8 bytes integer).", string));
            }
        }
    };
    ...
    {
        return new XXHashPartitioner.LongToken(Long.MAX_VALUE);
    }
