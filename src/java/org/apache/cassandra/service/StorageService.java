...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    public void waitForSchema(int delay)
    {
        ...
        while (totalTime < globalMigrationTaskWait) {
            ...
            logger.info("Initiating another schema agreement round ...");
            ...
            for (final UUID schema : schemaVersionsToWaitFor)
            {
                ...
                logger.info(String.format("Hosts for schema %s: %s", schema, hostsForSchema));

                final Set<InetAddress> intersection = Sets.intersection(MigrationTask.getSubmitted(), hostsForSchema);

                logger.info(String.format("Hosts there are already submitted migration requests for schema %s: %s", schema, new ArrayList<>(MigrationTask.getSubmitted())));

                logger.info(String.format("Intersection of hosts we sent migration messages to for schema %s and hosts we are waiting responses from %s: ",
                                   schema,
                                   intersection));
                ...
                logger.info(String.format("Going to pull a schema %s from node %s", schema, nextHost));
    ...
    private Set<UUID> getSchemasWaitingMigrationResponses() {
        ...
        logger.info("Finished migration schema responses until now " + finishedUpToNow);
        ...
        logger.info("Schemas of pending responses " + schemasOfPendingResponses);
    ...
    private void waitForNotEmptyLocalVersion(int delay)
    {
        ...
        while (totalTime < delay)
        {
            ...
            {
                logger.info("got not empty schema: {}, returning from waitForNotEmptyLocalVersion", version);
                return;
            }
